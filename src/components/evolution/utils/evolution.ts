...
const generateLicencePlate = (generationIndex: number | null, genomeIndex: number): CarLicencePlateType => {
...
export const genomeStringToGenome = (genomeString: string): Genome => {
  return genomeString.split(' ').map<Gene>((geneString: string) => {
    const gene: Gene = parseInt(geneString, 10) === 1 ? 1 : 0
    return gene
  })
}

export const formatCoefficient = (coefficient: number, shortNumber: boolean = true): number => {
  if (shortNumber) {
...
export const saveGenerationToStorage = (data: GenerationDataInStorage): boolean => {
  const { generation, generationIndex, lossHistory, avgLossHistory } = data
