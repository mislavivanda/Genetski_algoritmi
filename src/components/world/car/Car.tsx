...
function Car(props: CarProps) {
  ...
  const flWheelInfo = {
    ...wheelInfo,
    isFrontWheel: true,
    chassisConnectionPointLocal: [-CHASSIS_WHEEL_WIDTH / 2, CHASSIS_GROUND_CLEARANCE, CHASSIS_FRONT_WHEEL_SHIFT],
  }
  ...
  const blWheelInfo = {
    ...wheelInfo,
    isFrontWheel: false,
    chassisConnectionPointLocal: [-CHASSIS_WHEEL_WIDTH / 2, CHASSIS_GROUND_CLEARANCE, CHASSIS_BACK_WHEEL_SHIFT],
  }
  ...
  const label = withLabel ? (
    <span>
      Loss: <span style={{ color: distanceColor, fontWeight: 'bold' }}>{formatLossValue(carLoss)}</span>
    </span>
  ) : null
