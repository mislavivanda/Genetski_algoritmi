...
function DynamicCars(props: DynamicCarsProps) {
  ...
  const activeCars = cars.map((car) => {
    ...
    const onForward = () => {
      onEngineForward(carsAPIs.current[uuid].api)
    }
    const onBackward = () => {
      onEngineBackward(carsAPIs.current[uuid].api)
    }
    const onNeutral = () => {
      onEngineNeutral(carsAPIs.current[uuid].api)
    }
    const onLeft = () => {
      onWheelsLeft(carsAPIs.current[uuid].api)
    }
    const onRight = () => {
      onWheelsRight(carsAPIs.current[uuid].api)
    }
    const onStraight = () => {
      onWheelsStraight(carsAPIs.current[uuid].api)
    }
    const onBreak = () => {
      onPressBreak(carsAPIs.current[uuid].api)
    }
    const onBreakRelease = () => {
      onReleaseBreak(carsAPIs.current[uuid].api)
    }
    ...
    const carColor = withRandomColors ? getRandomColor() : performanceBoost ? CHASSIS_SIMPLIFIED_BASE_COLOR : undefined
