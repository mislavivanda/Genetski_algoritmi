...
function World(props: WorldProps) {
  ...
  const stats = withStat ? <Stats showPanel={0} /> : null
  ...
  const joystickController = withJoystickControl ? <CarJoystickController /> : null

  const keyboardController = withKeyboardControl ? <CarKeyboardController /> : null

  const cameraFov = window.innerWidth < WIDER_CAMERA_SCREEN_MAX_WIDTH ? 30 : 25

  const environment = performanceBoost ? null : <Environment background={false} preset={'night'} />
  return (
