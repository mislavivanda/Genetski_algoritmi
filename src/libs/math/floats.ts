...
function bitsToFloat(bits: Bits, precisionConfig: PrecisionConfig): number {
  ...
  const exponentUnbiased = exponentBits.reduce((exponentSoFar: number, currentBit: Bit, bitIndex: number) => {
  ...
  const fraction = fractionBits.reduce((fractionSoFar: number, currentBit: Bit, bitIndex: number) => {
