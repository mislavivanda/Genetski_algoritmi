...
function mate(father: Genome, mother: Genome, mutationProbability: Probability): [Genome, Genome] {
  ...
  return [mutate(firstChild, mutationProbability), mutate(secondChild, mutationProbability)]
}
...
export function select(generation: Generation, fitness: FitnessFunction, options: SelectionOptions) {
  const { mutationProbability, longLivingChampionsPercentage } = options
